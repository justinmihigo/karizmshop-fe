  ...
  return (
    ...
    >
      <div className="w-full  pl-0 pt-1 md:w-1/2 flex h-auto flex-col flex-1 lg:pl-10 md:pl-0 lg:pt-10 md:pt-10">
        ...
        <div className=' w-[100%] lg:w-[85%] md:w-[85%]'>
          <p className="text-[20px] justify-center items-center text-[#1C4A93] font-bold  md:text-[40px] lg:text-[40px] lg:justify-start lg:items-start">
            {slide.mainText}
          </p>
        </div>
      </div>
    ...
    <div className="relative w-full overflow-hidden">
      ...
      <div className="relative bottom-[0] left-1/2 transform -translate-x-1/2 space-x-2 p-5 flex flex-row justify-center items-center">
        {slides.map((_, index) => (
          <div
            key={`indicator-${index}`}
            className={`w-3 h-3 rounded-full cursor-pointer ${currentIndex === index ? 'bg-[#1C4A93]' : 'bg-[#ffffff] border border-black'}`}
