...
const mockCategories: Category[] = Array.from({ length: 50 }, (_, index) => ({
  ...
  parentCategory: index % 5 === 0 ? null : `Category ${Math.floor(index / 5) * 5 + 1}`,
          ...
          <tbody className="text-gray-600 text-sm font-light">
            {currentItems.map((category) => (
              <tr key={category.id} className="border-b border-gray-200 hover:bg-gray-100">
                ...
                <td className="py-3 px-6 text-left">{category.parentCategory || 'N/A'}</td>
                ...
                <td className="py-3 px-6 text-left">
                  <div className="flex items-center space-x-4">
                    <button onClick={() => handleEdit(category.id)} className="text-blue hover:text-blue-700 text-lg cursor-pointer">
                      < MdModeEdit className="w-5 h-5" color='#1C4A93' />
                    </button>
                    <button onClick={() => handleDelete(category.id)} className="text-red hover:text-red-700 text-lg cursor-pointer">
          ...
          {Array.from({ length: Math.ceil(categories.length / itemsPerPage) }, (_, i) => (
            <button
              ...
              className={`mx-1 px-3 py-1 rounded ${
                currentPage === i + 1 ? 'bg-[#1C4A93] text-white' : 'bg-gray-200'
              }`}
            >
              {i + 1}
            </button>
